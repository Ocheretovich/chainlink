...
func (cs AddAptosLanes) Apply(env deployment.Environment, cfg config.UpdateAptosLanesConfig) (deployment.ChangesetOutput, error) {
	...
	updateInputsByAptosChain := convertToUpdateAptosLanesSeqInput(state.AptosChains, cfg)
	for aptosChainSel, sequenceInput := range updateInputsByAptosChain {
		...
		updateSeqReport, err := operations.ExecuteSequence(env.OperationsBundle, UpdateAptosLanesSequence, deps, sequenceInput)
		if err != nil {
			return deployment.ChangesetOutput{}, err
		}
