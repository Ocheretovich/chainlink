	...
	var tt = []testCase{
		{
			...
			engineFactoryFn: func(ctx context.Context, wfid string, owner string, name shared.WorkflowName, config []byte, binary []byte) (services.Service, error) {
...
func Test_workflowPausedActivatedUpdatedHandler(t *testing.T) {
	t.Run("success pausing activating and updating existing engine and spec", func(t *testing.T) {
		...
		require.Equal(t, shared.WorkflowID(updatedWFID), engine.WorkflowID)
	})
}
