...
type WorkflowRegistryWorkflowUpdatedV1 struct {
	OldWorkflowID shared.WorkflowID
	WorkflowOwner []byte
	DonID         uint32
	NewWorkflowID shared.WorkflowID
	WorkflowName  string
	BinaryURL     string
	ConfigURL     string
	SecretsURL    string
...
type engineFactoryFn func(ctx context.Context, wfid string, owner string, name shared.WorkflowName, config []byte, binary []byte) (services.Service, error)
...
func WithStaticEngine(engine services.Service) func(*eventHandler) {
	return func(e *eventHandler) {
		e.engineFactory = func(_ context.Context, _ string, _ string, _ shared.WorkflowName, _ []byte, _ []byte) (services.Service, error) {
			return engine, nil
		}
	}
}
...
func (h *eventHandler) createWorkflowSpec(ctx context.Context, payload WorkflowRegistryWorkflowRegisteredV1) (*job.WorkflowSpec, error) {
	...
	if !shared.WorkflowID(hash).Equal(payload.WorkflowID) {
...
func (h *eventHandler) engineFactoryFn(ctx context.Context, workflowID string, owner string, name shared.WorkflowName, config []byte, binary []byte) (services.Service, error) {
...
func (h *eventHandler) tryEngineCreate(ctx context.Context, spec *job.WorkflowSpec) error {
	...
	workflowName, err := shared.NewWorkflowName(spec.WorkflowName)
	if err != nil {
	...
	wid, err := shared.WorkflowIDFromHex(spec.WorkflowID)
	if err != nil {
		return err
	}
